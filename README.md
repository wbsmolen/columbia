...
- `ohttp-gateway/` is a vendored copy of Cloudflare's [`privacy-gateway-server-go`](https://github.com/cloudflare/privacy-gateway-server-go) (BSD-3, the RFC 9458 reference gateway). The source carries three small, env-gated additions (a relay-auth check, an endpoint guard, and an optional outbound rate limiter) plus two unconditional response-path fixes (a response-side failure returns an inner `500` rather than a `400`, and the upstream client has a bounded timeout); everything else it does is configured at runtime through environment variables. Provenance, the full list of additions, and the required attribution live in [`ohttp-gateway/VENDORED.md`](./ohttp-gateway/VENDORED.md).

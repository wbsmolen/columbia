...
A sealed read can fail: the target is not on `ALLOWED_TARGET_ORIGINS` (the gateway returns a BHTTP `403`), the gateway's shared upstream budget is momentarily spent (`429` with `Retry-After`), or the path is down. When it does, the client decides how to degrade. *Fail-open*: fall back to a direct fetch over its own session, trading the split-trust property for availability. *Fail-closed*: surface the error and route nothing, keeping the privacy property even when the path is unavailable. Columbia takes no position; the choice is the client's policy.
...
- `ohttp-relay/` is a Node service with an optional locked Azure Table state adapter. It forwards `message/ohttp-req` bodies to the gateway in a fresh request that carries no client headers and no `X-Forwarded-For`. As the one public surface it also carries the abuse controls (per-IP rate limiting, a concurrency cap, a strict request shape, and a pluggable client-auth hook), with transient IP limits and optional durable anonymous spend hashes; neither is logged. For the hardened topology it can run as the sole public hop, with the gateway and cache on internal ingress, and pass the gateway's public key config through to clients. See [SELFHOSTING.md](./SELFHOSTING.md). Its logs are RED metrics only.
- `ohttp-gateway/` is a vendored copy of Cloudflare's [`privacy-gateway-server-go`](https://github.com/cloudflare/privacy-gateway-server-go) (BSD-3, the RFC 9458 reference gateway). Local modifications include relay authentication, an endpoint guard, an optional outbound rate limiter, response-side error mapping, a bounded upstream timeout, bounded operational summaries and corrected Prometheus accounting. Request diagnostics exclude caller-derived content. Runtime environment variables configure deployment-specific behavior. Provenance, the full list of additions, and the required attribution live in [`ohttp-gateway/VENDORED.md`](./ohttp-gateway/VENDORED.md).
...
- A relay failure adds a bounded `reason` and error code describing the relay-to-gateway hop, so a 502 can be attributed to a cause without naming a client or a target. The reason vocabulary is fixed, so cardinality stays bounded.
- The gateway logs operational events only, and with `LOG_SECRETS=false` the HPKE seed never reaches stdout.
- Default application logs provide throughput, error rate, latency, cache hit-rate, and per-component health without recording browsing targets or client identifiers. Per-request timestamps and durations remain visible; these schemas do not establish anonymity or describe separately configured platform logs.
...
- The relay receives ciphertext and strips client headers before forwarding; the gateway decrypts the request without receiving the client's IP from the relay. This separation depends on the configured routing and non-colluding operators. A common operator can combine observations, and timing and size metadata can still support correlation.

...
The original four deviations from upstream are: (1) the GCP App Engine example manifests (`gateway.yaml`, `gateway-protohttp.yaml`, `app.yaml`) were removed (see below); (2) two small env-gated access controls were added to `main.go` (see "Added: relay auth + endpoint guard" below); (3) an optional global outbound rate limiter was added as a new file, `ratelimit.go` (see below), with a call site in `handler.go`; (4) two unconditional behavior fixes on the response path, in `handler.go` and `main.go` (see "Changed: response-side error mapping and upstream timeout" below). The additional transport-observability changes described below are also local modifications. Other deployment-specific behavior is supplied at runtime through environment variables (documented in the upstream `README.md`), not by patching the source:
...
The vendored dependency tree tracks the upstream commit above, plus documented security-patch bumps to indirect dependencies (see `CHANGELOG.md` — e.g. the `golang.org/x/crypto`/`x/sys` bump that cleared a set of Dependabot alerts). The upstream commit identifies the vendored base, not the current modified source; the changes listed here and the documented dependency bumps must be preserved when refreshing that base.

## Added: relay auth, endpoint guard, and rate limiter (`main.go`, `handler.go`, `ratelimit.go`)

Three env-gated additions, each inert unless an operator turns it on:
...
## Changed: response-side error mapping and upstream timeout (`handler.go`, `main.go`)

Two fixes on the response path, added in v1.4.4. Unlike the three above these are not env-gated, so they apply to every deployment:

- **A response-side failure maps to `500`, not `400`.** Upstream returns `PayloadMarshallingError` — an inner `400` — when the target's response fails to translate or marshal, which includes the case where the target's body errors mid-read. A client fails open on a 5xx but reasonably treats a 4xx as the target's own answer, so the upstream mapping turned a transient upstream hiccup into a hard, user-visible failure. Both the binary-HTTP and protobuf paths now return `GatewayInternalServerError` instead. Request-side decode failures are untouched and still return `400`. `handler_test.go` is a Columbia-added regression test covering both paths.
- **A bounded upstream timeout.** The `http.Client` the gateway fetches targets with is constructed with a 30s `Timeout`. Upstream leaves it unbounded, so a stalled target could pin a gateway worker indefinitely.

## Added: bounded operational diagnostics

The post-v1.4.4 review adds `summary_metrics.go`, wraps the configured metrics exporter in `main.go`, and records actual target HTTP statuses in `handler.go`. Active minute summaries retain only fixed event/stage/outcome values and disjoint duration buckets, with the actual collection interval. Outer HTTP success and upstream refusal remain separate stages. Summaries do not contain request URLs, headers, client identities or request IDs.

`prometheus_metrics.go` now registers the histogram once at startup, reuses an existing compatible collector, returns invalid configuration at startup, and records seconds to match the default histogram buckets. Its former per-request registration and millisecond observations were incorrect.

Request-path debug logs in `gateway.go`, `handler.go` and the health handler no longer include caller-supplied paths, methods, header values or free-form target errors. This does not make reflective debug endpoints or `LOG_SECRETS=true` suitable for production; keep those explicitly disabled. `GATEWAY_DEBUG` still controls caller-facing diagnostic response text.

Regression coverage lives in `summary_metrics_test.go`, `prometheus_metrics_test.go` and `request_logging_test.go`. The builder also tracks Go 1.27 and builds for Docker's requested target OS/architecture; it does not alter gateway key derivation or wire formats.

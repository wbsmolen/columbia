...
- A relay failure adds a bounded `reason` and the underlying error code, so a 502 is attributable to a cause: `gw_error` (the request to the gateway errored, with whether the socket was `reused`), `gres_error` (the gateway errored mid-response), `resp_too_large` (over `MAX_RESP_BYTES`, with the byte count), and `gw_retry` for the one-shot retry on a reset keep-alive socket. A crash logs its trace before exiting non-zero. All of these describe the relay-to-gateway hop, so none of it is request-derived: the reason set is a fixed, bounded vocabulary that names no client and no target.
...
- The gateway logs operational events only at its default `LOG_LEVEL=info`, and `LOG_SECRETS=false` keeps the HPKE seed out of stdout.
- This holds by construction at the default log level, not merely by policy — but it has one operator-controlled exception: setting `LOG_LEVEL=debug` on the gateway turns on `slog.Debug` lines that include the target `Host` and `URL` for a couple of error paths (a forbidden-target rejection, a rate-limit rejection). `GATEWAY_DEBUG` is a separate knob with a similar effect on error responses rather than logs. Never set either in production; see SELFHOSTING.md.
- This is sufficient to operate the system: throughput, error rate, latency, cache hit-rate, and per-component health, while there is structurally no way to log a reading history at the default level. Privacy derives from what is never written, not from a redactor scrubbing logs after the fact.

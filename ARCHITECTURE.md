...
| Gateway | no | yes | the relay sends a fresh request with no client IP or headers; the gateway holds the HPKE seed. When an anonymous app-level credential is routed, it also sees that credential, which names the app, not the client |
...
- Identity-bound auth and writes are out of scope by design; anonymous app-level reads are not. A request tied to the client's identity (a login session, a per-user token, any write) must stay on the client's own session: routing it through the gateway would place an identity-bound credential in front of the operator and defeat the model. An **anonymous, app-level** credential is different: it is shared across all users and names the application, not the client, so a client MAY seal it into the BHTTP request and have the gateway forward it to an allowlisted host. In that case the gateway additionally sees the app-level credential (it already sees request content); because the credential does not link to a client identity, the relay-holds-identity / gateway-holds-content split is preserved. See [README.md](./README.md#what-its-for).
...
- Relay failures add a bounded reason and normalized transport error code. `rate_limit` distinguishes per-client budget from `capacity` exhaustion; `gw_error`, `gres_error`, and `resp_too_large` distinguish request, response-stream and size failures. Unknown routes are `other`; crash records carry only bounded error types/codes. Opaque requests are never automatically replayed after an ambiguous reset.
- The gateway emits one `operational_summary` per active minute, supplementing optional StatsD/Prometheus. Stage `outer` counts final HTTP responses; `upstream` counts target HTTP statuses; `payload` counts the OHTTP translation result; `result` counts pipeline outcomes. **Do not add stages to calculate requests:** one request produces several stages. Histograms use elapsed time from request start in disjoint buckets bounded at 10, 100, 1000, 5000 and 15000 ms. No URL, origin, method, request ID, session identifier or per-request timestamp is retained in these summaries. A 200 outer response does not imply the target accepted the request.
- Successful `GET /health` probe hits are not logged: at platform probe cadence they are almost all log volume (in the reference deployment, ~98% of ingested rows), drowning the RED signal and billing the log store for noise. `LOG_HEALTH=1` re-enables them for a debugging session; a failing probe is unaffected either way.
- Gateway summaries use `LOG_LEVEL=info`; `LOG_SECRETS=false` keeps the HPKE seed out of stdout.
- The forbidden-target and rate-limit debug logs no longer include target Host/URL. Other debug paths and `GATEWAY_DEBUG` error responses are for local investigation; keep debug modes and `LOG_SECRETS` disabled in production.
- These default application-log schemas support throughput, error rate, latency, cache hit-rate, and per-component health while excluding browsing targets and client identifiers. They do not establish anonymity: outer-request timing and size metadata remain observable, platform logging is configured separately, and a common operator can combine relay and gateway observations.
...
- Routed app-level reads share one egress budget. A cache hit fans out across clients, but an anonymous app-level read routed to a live API does not: every such fetch leaves from the single gateway egress IP under one shared credential, so the target sees one caller against one global budget. That budget does not grow with users. Size it against the target's published limit and cap outbound volume with the gateway's `GATEWAY_MAX_QPM`. Unlike the cache tier, this path has no per-user multiplier to amortize; it is a shared bottleneck by construction. See [SELFHOSTING.md](./SELFHOSTING.md#shared-egress-one-ip-one-credential-one-budget).

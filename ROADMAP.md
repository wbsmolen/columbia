...
v1.4.4 covered the relay-to-gateway hop and the client-visible failure mode. The relay pools that hop over a keep-alive `https.Agent` and retries the gateway `POST` exactly once, on a fresh socket, when a kept-alive socket is reset by the peer and nothing has yet been sent to the client — safe to replay because the request body is fully buffered. The gateway now maps a response-side failure (a target body that errors mid-read) to an inner `500` instead of a `400`, so a client that fails open on 5xx degrades gracefully rather than treating a transient upstream hiccup as a hard answer, and its upstream `http.Client` has a 30s timeout where it previously had none. The relay also survives what used to kill it: an error on the gateway response stream is handled rather than thrown as an unhandled `'error'` event, and top-level `uncaughtException`/`unhandledRejection` handlers log a trace before exiting non-zero. All of it is RED-observable — every 502 carries a `reason` (`gw_error`, `gres_error`, `resp_too_large`) and the error code, and a retry logs `gw_retry`.

Still open: the gateway's fetch to the target and the cache's fetch upstream are both still single-attempt, so retrying covers only the relay-to-gateway leg; the one retry is immediate, with no backoff or jitter and no general retry budget; and nothing circuit-breaks on a target that is failing steadily. Per-call timeouts are the one part now in place on all three hops (`GW_TIMEOUT_MS` at the relay, 30s at the gateway, a 10s `AbortSignal.timeout` at the cache).
